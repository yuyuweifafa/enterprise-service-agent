 ...
 *
 * 为什么必须有这一层 —— 主管线的隐含假设是「所有输入都是小助当前服务范围内的请求」，
 * 于是它总能找到一个"最接近"的意图。实测踩到的三个坑：
 *
 *   「今天天气怎么样」  → 沉淀知识缺口，回复「将由内容运营补充相关制度」
 *   「帮我写段冒泡排序」→ 识别成薪酬咨询，误判为企业服务请求
 ...
 *   in_scope      正常服务请求  → 进主管线（知识缺失时才沉淀；IT / 行政可转人工）
 *
  ...
  if (st?.enabled) {
    if (!hasMeaningfulChars(normalized)) {
      return {
        ...
        reply: '没看懂你的意思，能具体说说要办什么吗？比如「帮我看看今天有什么待办」「我明天要客户评审，帮我检查准备事项」。',
      };
    }
    if (normalized.length < (st.minMeaningfulChars ?? 2)) {
      return {
        ...
        reply: '能再说具体一点吗？比如「帮我看看今天有什么待办」「我要出差去成都，帮我把差旅和报销准备好」。',
      };
    }
  }
